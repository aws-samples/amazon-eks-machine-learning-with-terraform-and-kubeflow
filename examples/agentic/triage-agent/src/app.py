"""
KAgentApp wrapper for Triage Agent.

Wraps the LangGraph agent with kagent's KAgentApp to:
...
Memory is always enabled — the triage agent's primary function depends on it.
"""
...
def load_memory_tools() -> list:
    """
    Load memory tools for the triage agent.
...
def _init_otel():
    ...
    try:
        ...
        # Pod env exposes the HTTP endpoint (:4318); gRPC exporter needs :4317
        grpc_endpoint = endpoint.replace(":4318", ":4317").replace("http://", "")
        resource = Resource.create({
            "service.name": os.getenv("OTEL_SERVICE_NAME", "triage-agent"),
        })
...
def main():
    ...
    # Load memory tools (always enabled for triage agent)
    ...
    tools = memory_tools
    if not tools:
        logger.warning("Running in Q&A mode (no memory tools) — triage functionality limited")
    ...
    logger.info(f"Starting Triage Agent on {host}:{port}")
